...
import type { AgentOAuthAuthorizeParams, AgentOAuthConsentResponseBody } from "@/types";
...
const MESSAGES = {
  ...
  body: "Request body must be a JSON object with params and a decision",
...
function redirectResponse(redirectUrl: string): Response {
  ...
  return oauthJson(body, HTTP.ok, AGENT_OAUTH_NO_STORE_HEADERS);
}
...
function toConsentRequest(body: unknown): ConsentRequest | null {
  ...
  return { params, decision: body.decision, scopes: body.scopes, expiresInDays: body.expiresInDays };
}
...
async function readConsentRequest(request: Request): Promise<Parsed<ConsentRequest>> {
  ...
  if (!read.ok) {
    const response =
      read.reason === "too_large"
        ? errorResponse(MESSAGES.tooLarge, HTTP.payloadTooLarge)
        : errorResponse(MESSAGES.body, HTTP.badRequest);
  ...
  try {
    ...
    return { ok: false, response: errorResponse(MESSAGES.body, HTTP.badRequest) };
  }
  const parsed = toConsentRequest(body);
  if (parsed === null) return { ok: false, response: errorResponse(MESSAGES.body, HTTP.badRequest) };
...
async function approve(
  ...
  if (!access.ok) return errorResponse(access.message, HTTP.badRequest);
  ...
  switch (created.kind) {
    ...
    case "grant_cap":
      return errorResponse(MESSAGES.grantCap, HTTP.conflict);
    case "invalid_client":
      return errorResponse(MESSAGES.invalidRequest, HTTP.badRequest);
    case "unavailable":
      return errorResponse(MESSAGES.unavailable, HTTP.unavailable);
  }
}

export async function POST(request: Request): Promise<Response> {
  if (!isMcpOAuthEnabled()) return oauthNotFound();
  if (!isSameOriginJson(request)) return errorResponse(MESSAGES.forbidden, HTTP.forbidden);
  const userId = await getSessionUserId();
  if (userId === null) return errorResponse(MESSAGES.unauthorized, HTTP.unauthorized);
