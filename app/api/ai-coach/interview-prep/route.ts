...
export async function POST(request: NextRequest) {
  try {
    ...
    const preparation = await aiCoach.prepareForInterview(jobDescription, userBackground)

    return NextResponse.json({ preparation })
  } catch (error) {
    console.error("Error in interview preparation:", error)
    return NextResponse.json({ error: "Failed to generate interview preparation" }, { status: 500 })
  }
}
