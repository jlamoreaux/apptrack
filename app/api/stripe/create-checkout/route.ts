...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Creating checkout for user ${user.id}, plan ${planId}, billing ${billingCycle}`)
    ...
    if (!priceId) {
      return NextResponse.json({ error: "Price ID not found for billing cycle" }, { status: 400 })
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `${request.headers.get("origin")}/dashboard/upgrade/success?session_id={CHECKOUT_SESSION_ID}`,
