...
- **Entitlement collapse to one boolean.** Today gating keys off the plan-name string `"AI Coach"` across `lib/constants/permissions.ts`, `lib/utils/plan-helpers.ts`, `lib/constants/plans.ts`, and `lib/middleware/permissions.ts` (`getUserPlanInfo` already returns `{isPro, isAICoach, isFree}`). Introduce a single canonical `isPro(planName: string)` — true for the new Pro plan AND grandfathered legacy paid plans (Pro + AI Coach) — and route every AI-feature gate through it. **Contract:** `isPro` takes a resolved plan-name string, not a subscription object; entitlement by status (active/trialing vs canceled/past_due) is enforced upstream by `getSubscription`, which only returns active/trialing rows (an inactive row resolves to Free). Roast stays ungated (already is; only rate-limited).
...
- **Authorization (in addition to RLS):** RLS is service-role-only, so every API route must ALSO scope by the session `user_id` on every read/write (`.eq("user_id", user.id)`), never trusting a client-supplied id; a row that isn't the caller's returns 404. Account deletion cascades (FK `on delete cascade` to `profiles`). Tests must include cross-user denial (a row owned by another user → 404), not only privileged service-role access.
...
- **Acceptance:** a new Pro user reaches a generated starter case within 5 min of signup; **a Free user also reaches a starter case** — Zero-to-Case is free once for everyone (the AI/Pro gate does not apply to this one onboarding call); a logged win appears in Friday's recap; PostHog funnels exist for every step.
