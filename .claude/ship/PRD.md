...
**Source of truth:** `/Users/jlmx/Downloads/apptrack-career-companion-phase0-prd.md` (owner PRD). This document translates it into a concrete build plan grounded in the actual codebase. **Phase 2 (coaching loop prototype) is explicitly out of scope — gated on Phase 0 results + owner approval.**
...
- **Analytics**: after each successful batch, fire `career_email_sent` per recipient with `{ campaign: 'career_companion_validation' }`. Distinct id: `user_id` when present, else the email address (`audience_members.user_id` is nullable) — documented so the insight reads counts, not identity joins. To avoid one HTTP request per event (`posthog-server.ts` uses `flushAt: 1`), the send route constructs a **local batched `PostHog` client** (`flushAt: 100`) and awaits `shutdown()` before responding. Send count is also persisted on the `campaign_sends` row — the durable denominator for the gate.
...
- New `lib/analytics/career-events.ts` mirroring `lib/analytics/campaign-events.ts` (private capture helper merging `...getStoredUTMParams()`, typed track functions). Events:
  ...
  - `career_waitlist_joined` — **server-side in the API route, using the client-forwarded `ph_distinct_id` as distinct id when present** (falls back to `user_id`, then email). This stitches the join to the same browser session as `viewed`, making the viewed → joined funnel work for anonymous visitors, while staying ad-blocker-resistant. Properties: `review_timing`, `source`, whitelisted UTMs (client-forwarded, therefore spoofable — acceptable; the DB is ground truth).
...
- **PII**: waitlist stores email + review timing only; events carry `review_timing` + UTMs.
...
16. **Null `user_id` distinct-id ambiguity** — specified: `user_id` else email for `career_email_sent`; `ph_distinct_id` else `user_id` else email for `career_waitlist_joined`.
