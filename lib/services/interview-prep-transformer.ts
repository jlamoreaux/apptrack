...
export class InterviewPrepTransformerService {
  private transformationCache = new Map<string, {
   ...
   */
  private async performTransform(request: TransformationRequest): Promise<TransformationResult> {
    ...
    try {
      ...
      const fallbackContent = this.createFallbackContent(request.content, request.structured)
    ...
    if (typeof content === 'string') {
      if (structured) {
        ...
        if (!this.validateStructuredContent(parsed)) {
          ...
          if (this.config.FEATURES.ENABLE_DETAILED_LOGGING) {
            console.warn('Parsed content failed validation, returning original string')
          }
          return content
        }
  ...
  private extractCompanyName(jobDescription: string): string {
    try {
      const limitedDescription = jobDescription.slice(0, this.config.PARSING.MAX_JOB_DESCRIPTION_LENGTH)

      for (const pattern of this.config.PATTERNS.COMPANY_EXTRACTION) {
        const match = limitedDescription.match(pattern)
        if (match && match[1]) {
          const company = match[1].trim()
          if (InterviewPrepValidation.isValidCompanyName(company)) {
            return company
          }
        }
      }
  ...
  private createFallbackContent(
    ...
    if (structured) {
      ...
      return {
        ...
        generalTips: ['Be confident and prepared', 'Research the company thoroughly'],
        companyInsights: ['Learn about company culture and values'],
        roleSpecificAdvice: ['Understand the specific role requirements'],
        practiceAreas: ['Common behavioral questions', 'Role-specific scenarios'],
  ...
  private generateCacheKey(content: string, jobContext: JobContext): string {
    ...
    const contextHash = this.simpleHash(`${jobContext.company}-${jobContext.role}`)
