 ...
 */
export function formatAnalysisForExport(analysis: JobFitAnalysisResult): string {
 ...
 */
export function formatAnalysisForRichText(analysis: JobFitAnalysisResult): string {
  const formatDate = (dateString: string) => {
    return new Date(dateString).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
...
<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px;">
  <div style="text-align: center; border-bottom: 2px solid #2563eb; padding-bottom: 15px; margin-bottom: 20px;">
    ...
    <p style="color: #6b7280; margin: 8px 0; font-size: 14px;">Generated: ${formatDate(analysis.generatedAt)}</p>
  </div>
  ...
  <div style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border: 1px solid #93c5fd; border-radius: 8px; padding: 15px; margin: 15px 0; text-align: center;">
    <div style="font-size: 36px; font-weight: bold; color: ${scoreColor}; margin: 8px 0;">${analysis.overallScore}%</div>
    <div style="font-size: 16px; font-weight: 600; color: #374151;">${analysis.scoreLabel}</div>
  </div>
  ...
  <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 15px 0;">
    <div style="text-align: center; padding: 10px; background: #f9fafb; border-radius: 6px; border: 1px solid #e5e7eb;">
      ...
      <div style="font-size: 18px; font-weight: bold; color: #1f2937;">${analysis.matchDetails?.skillsMatch || 0}%</div>
    </div>
    <div style="text-align: center; padding: 10px; background: #f9fafb; border-radius: 6px; border: 1px solid #e5e7eb;">
      <div style="font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 4px;">Experience Match</div>
      <div style="font-size: 18px; font-weight: bold; color: #1f2937;">${analysis.matchDetails?.experienceMatch || 0}%</div>
    </div>
    <div style="text-align: center; padding: 10px; background: #f9fafb; border-radius: 6px; border: 1px solid #e5e7eb;">
      <div style="font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 4px;">Education Match</div>
      <div style="font-size: 18px; font-weight: bold; color: #1f2937;">${analysis.matchDetails?.educationMatch || 0}%</div>
    </div>
  </div>
 ...
 */
export async function copyAnalysisToClipboard(analysis: JobFitAnalysisResult): Promise<void> {
  ...
  if (navigator.clipboard && navigator.clipboard.write) {
    try {
      ...
      console.warn('Rich text clipboard failed, falling back to plain text:', error);
    }
  }
...
export async function downloadAnalysisPDF(
  ...
  try {
    ...
    const addWrappedText = (text: string, x: number, y: number, maxWidth: number, fontSize: number = 12, isBold: boolean = false): number => {
    ...
    if (applicationInfo) {
      pdf.setFontSize(16);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`${applicationInfo.role} at ${applicationInfo.company}`, pageWidth / 2, currentY, { align: 'center' });
      currentY += 10;
    }
    ...
    pdf.text(`Generated on ${formatDate(analysis.generatedAt)}`, pageWidth / 2, currentY, { align: 'center' });
    currentY += 20;
    ...
    else if (analysis.overallScore >= 65) pdf.setTextColor(217, 119, 6); // Orange
    else pdf.setTextColor(220, 38, 38); // Red
    
    pdf.text(`${analysis.overallScore}%`, pageWidth / 2, currentY + 15, { align: 'center' });
    
    pdf.setTextColor(0, 0, 0); // Reset to black
    pdf.setFontSize(16);
    pdf.setFont('helvetica', 'bold');
    pdf.text(analysis.scoreLabel, pageWidth / 2, currentY + 25, { align: 'center' });
    ...
    if (analysis.matchDetails) {
      ...
      const breakdownData = [
        { label: 'Skills Match', value: analysis.matchDetails.skillsMatch || 0 },
        { label: 'Experience Match', value: analysis.matchDetails.experienceMatch || 0 },
        { label: 'Education Match', value: analysis.matchDetails.educationMatch || 0 }
      ];
      ...
      breakdownData.forEach((item, index) => {
        ...
        pdf.setFontSize(10);
        pdf.setFont('helvetica', 'bold');
        pdf.text(item.label.toUpperCase(), x + columnWidth / 2, currentY + 8, { align: 'center' });
        
        // Value
        pdf.setFontSize(18);
        pdf.setFont('helvetica', 'bold');
        pdf.text(`${item.value}%`, x + columnWidth / 2, currentY + 16, { align: 'center' });
      });
    ...
    // Sections with data
    const sections = [
      { title: 'Your Strengths', data: analysis.strengths, color: [5, 150, 105] },
      { title: 'Areas to Address', data: analysis.weaknesses, color: [220, 38, 38] },
      { title: 'Key Requirements', data: analysis.keyRequirements, color: [37, 99, 235] },
      { title: 'Recommendations', data: analysis.recommendations, color: [124, 58, 237] }
    ];
    ...
    sections.forEach((section) => {
      ...
      if (section.data && section.data.length > 0) {
        section.data.forEach((item) => {
          ...
          // Item background
          pdf.setFillColor(249, 250, 251);
          const itemHeight = Math.max(15, pdf.splitTextToSize(item, contentWidth - 20).length * 6);
          ...
          pdf.setFontSize(11);
          pdf.setFont('helvetica', 'normal');
          currentY = addWrappedText(item, margin + 10, currentY + 5, contentWidth - 20, 11);
          currentY += 5;
        });
      } else {
        if (currentY > pageHeight - 30) {
    ...
    pdf.setFontSize(10);
    pdf.setFont('helvetica', 'bold');
    pdf.text('This report was generated by AppTrack AI Coach', pageWidth / 2, currentY, { align: 'center' });
    currentY += 6;
    
    pdf.setFont('helvetica', 'normal');
    pdf.text('For the most up-to-date analysis, visit your AppTrack dashboard', pageWidth / 2, currentY, { align: 'center' });
    currentY += 6;
    ...
    const filename = applicationInfo 
      ? `job-fit-analysis-${applicationInfo.company.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}-${applicationInfo.role.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}.pdf`
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            ${applicationInfo ? `<div class="company-info">${applicationInfo.role} at ${applicationInfo.company}</div>` : ''}
...
<head>
    ...
    <style>
        ...
        .score {
            font-size: 48px;
            font-weight: bold;
            color: ${analysis.overallScore >= 85 ? '#059669' : 
                   analysis.overallScore >= 75 ? '#2563eb' : 
                   analysis.overallScore >= 65 ? '#d97706' : '#dc2626'};
            margin: 10px 0;
        }
...
<body>
    <div class="header">
        <h1>Job Fit Analysis Report</h1>
        ${applicationInfo ? `<div class="subtitle">${applicationInfo.role} at ${applicationInfo.company}</div>` : ''}
        <div class="subtitle">Generated on ${formatDate(analysis.generatedAt)}</div>
    </div>
  ...
  try {
    ...
    const fileName = applicationInfo 
      ? `job-fit-analysis-${applicationInfo.company.replace(/\s+/g, '-').toLowerCase()}-${applicationInfo.role.replace(/\s+/g, '-').toLowerCase()}.html`
