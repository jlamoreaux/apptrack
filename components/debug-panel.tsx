...
export function DebugPanel() {
  ...
  const testSubscriptionTables = async () => {
    ...
    try {
      ...
      const { data: subs, error: subsError } = await supabase.from("user_subscriptions").select("*").limit(1)
      ...
      const { data: usage, error: usageError } = await supabase.from("usage_tracking").select("*").limit(1)
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <div className="flex gap-2">
            ...
            <Button onClick={testSubscriptionTables} size="sm" variant="outline">
              Test Subs
            </Button>
