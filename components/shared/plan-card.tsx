...
import { Check, Crown, Infinity, Heart, Bot, Sparkles, Brain, FileText, MessageSquare, Target } from "lucide-react"
...
export function PlanCard({
  ...
  const isPro = planName === PLAN_NAMES.PRO
  ...
  const cardContent = (
    <>
      ...
      <CardHeader className="pb-4">
        ...
        <CardDescription className={theme.colors.muted}>{subtitle}</CardDescription>

        <div className={`text-2xl font-bold ${theme.colors.text}`}>
          {price ? (
            <>
              ${price.amount}
              <span className={`text-sm font-normal ${theme.colors.muted}`}>/{price.period}</span>
            </>
          ) : (
            <>
              $0<span className={`text-sm font-normal ${theme.colors.muted}`}>/month</span>
            </>
          )}
        </div>
      </CardHeader>
    ...
    >
      ...
      <div className="text-center mb-6">
        <div className="flex items-center justify-center mb-2">
          ...
          <h3 className={`font-semibold text-lg ${theme.colors.text}`}>{title}</h3>
        </div>
