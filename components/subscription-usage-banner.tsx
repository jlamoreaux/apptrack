          ...
          <div className="flex-1">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span>
                  {usage?.applications_count || 0} / {subscription?.subscription_plans?.max_applications || 5}
                </span>
              </div>
              <Progress value={percentage} className="h-2" />
              {isAtLimit ? (
                <p className="text-sm text-red-600">
                  {"You've reached your application limit. Upgrade to Pro to add unlimited applications."}
                </p>
              ) : isNearLimit ? (
                <p className="text-sm text-yellow-700">
                  Only {remaining} applications remaining. Consider upgrading to Pro for unlimited applications.
                </p>
              ) : (
